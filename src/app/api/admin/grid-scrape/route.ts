...
const FOOD_TYPES = ["restaurants","cafe coffee shop","dhaba","bakery sweet shop","fast food","tea stall chai","juice soda shop","hotel dining"];
...
export async function GET() {
  const scrapeRows = (await query(`SELECT query,MAX(started_at) AS last_scraped FROM scrape_jobs WHERE status='completed' GROUP BY query`)).rows;
  const scrapeMap: Record<string,string> = {};
  for(const r of scrapeRows) scrapeMap[r.query]=r.last_scraped;

  const density = (await query(`SELECT a.name,COUNT(*)::int AS cnt FROM businesses b JOIN areas a ON a.id=b.area_id WHERE b.status='approved' GROUP BY a.name`)).rows;
  ...
  const cells = CELLS.map(cell=>{
    ...
    const daysSince=lastScraped?Math.floor((Date.now()-new Date(lastScraped).getTime())/86400000):null;
    const status=daysSince===null?"unscraped":daysSince<14?"fresh":daysSince<60?"stale":"outdated";
    return {...cell,lastScraped,daysSince,status,approvedCount:densityMap[cell.label]||0,queryCount:FOOD_TYPES.length};
  });
...
export async function POST(request: NextRequest) {
  ...
  for(const type of FOOD_TYPES){
    ...
    try{
      const sr=await fetch(`${base}/api/search?q=${encodeURIComponent(sq)}&fetchAll=true&strict=false`,{headers:{Cookie:cookie}});
      const sd=await sr.json();
      if(!sr.ok||!sd.businesses?.length) continue;
      totalFound+=sd.businesses.length;
      const ir=await fetch(`${base}/api/admin/businesses/ingest`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:cookie},body:JSON.stringify({searchQuery:sq,businesses:sd.businesses})});
      const id=await ir.json();
      totalNew+=id.newRecords||0; totalDup+=id.duplicates||0;
    }catch{}
  }
  return NextResponse.json({cellId,label:cell.label,queriesRun:FOOD_TYPES.length,found:totalFound,newRecords:totalNew,duplicates:totalDup});
}
