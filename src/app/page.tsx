...
function DashboardMock({ kind }: { kind: "admin" | "scraped" | "company" | "restaurants" | "field" }) {
  if (kind === "admin") {
    const rows: Array<{ name: string; area: string; status: "approved" | "pending" | "rejected" }> = [
      { name: "Chai Chatt", area: "PECHS, Karachi", status: "approved" },
      { name: "Bella Vita", area: "DHA, Karachi", status: "pending" },
      { name: "Cocochan", area: "Clifton, Karachi", status: "approved" },
      { name: "Unnamed Dhaba #12", area: "Gulshan, Karachi", status: "rejected" },
      { name: "Okra", area: "Clifton, Karachi", status: "pending" },
    ];
    return (
      <div className="p-4 sm:p-5">
        <div className="flex items-center justify-between mb-4">
          <span className="text-sm text-white/90 font-semibold">Admin · Dashboard</span>
          <span className="font-[family-name:var(--font-mono)] text-[10px] text-white/40">139 total businesses</span>
        </div>
        <div className="grid grid-cols-4 gap-2 mb-4">
          <MockStat k="Total" v="139" />
          <MockStat k="Pending" v="12" />
          <MockStat k="Approved" v="118" />
          <MockStat k="Rejected" v="9" />
        </div>
        <div className="rounded-lg border border-white/10 overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 bg-white/[0.04] font-[family-name:var(--font-mono)] text-[9px] uppercase tracking-wider text-white/40">
            <span>Pending Approval Queue</span>
            <span>Status</span>
          </div>
          {rows.map((r) => (
            <div key={r.name} className="flex items-center justify-between px-3 py-2 border-t border-white/5">
              <div>
                <p className="text-xs text-white/90 font-medium">{r.name}</p>
                <p className="text-[10px] text-white/40">{r.area}</p>
              </div>
              <MockBadge status={r.status} />
            </div>
          ))}
        </div>
      </div>
  ...
  if (kind === "scraped") {
    const rows = [
      { q: "hotel dining", area: "Tariq Road", found: 14 },
      { q: "juice / soda shop", area: "Tariq Road", found: 8 },
      { q: "tea stall", area: "Tariq Road", found: 21 },
    ];
    return (
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs text-white/90 font-semibold">Scraping Center · New Batches</span>
          <MockBadge status="new" />
        </div>
        <div className="space-y-2">
          {rows.map((r) => (
            <div key={r.q} className="flex items-center justify-between rounded-md bg-white/[0.03] border border-white/10 px-3 py-2">
              <div>
                <p className="text-[11px] text-white/90 capitalize font-medium">{r.q}</p>
                <p className="text-[9px] text-white/40 font-[family-name:var(--font-mono)]">{r.area}, Karachi</p>
              </div>
              <span className="font-[family-name:var(--font-mono)] text-[10px] text-[var(--forest-bright)]">{r.found} found</span>
            </div>
          ))}
        </div>
      </div>
  ...
  if (kind === "company") {
    ...
    return (
      <div className="p-4 sm:p-5">
        <div className="flex items-center justify-between mb-4">
          <span className="text-sm text-white/90 font-semibold">Company · Territory CRM</span>
          <span className="font-[family-name:var(--font-mono)] text-[10px] text-white/40">DHA &amp; Clifton</span>
        </div>
        <div className="grid grid-cols-4 gap-2 mb-4">
          <MockStat k="Restaurants" v="46" />
          <MockStat k="CRM Entries" v="15" />
          <MockStat k="Employees" v="6" />
          <MockStat k="Active Leads" v="15" />
        </div>
          ...
          {cols.map((c) => (
            <div key={c.label} className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
              ...
              <div className="space-y-1.5">
                {Array.from({ length: c.n > 3 ? 3 : c.n }).map((_, i) => (
                  <div key={i} className="h-6 rounded bg-white/[0.05] border border-white/5" />
                ))}
              </div>
              <p className="mt-1.5 font-[family-name:var(--font-mono)] text-[9px] text-[var(--forest-bright)]">{c.n} leads</p>
            </div>
          ))}
        </div>
      </div>
...
function BrowserFrame({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`pk-tilt-card rounded-xl overflow-hidden border border-white/10 bg-[#070B09] shadow-2xl shadow-black/40 hover:shadow-black/60 ${className}`}
    >
      <div className="flex items-center gap-1.5 px-3.5 py-2.5 bg-[#0C120F] border-b border-white/10">
