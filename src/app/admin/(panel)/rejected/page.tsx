...
import { getBusinesses, patchBusiness, deleteBusinessForever, bulkBusinessAction, getAreas, getCategories } from "@/lib/admin-api";
...
export default function RejectedRecordsPage() {
  ...
  const [busyId, setBusyId] = useState<string|null>(null); const [bulkBusy, setBulkBusy] = useState(false);
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [q, setQ] = useState(""); const [areaId, setAreaId] = useState(""); const [catId, setCatId] = useState("");
  const [areas, setAreas] = useState<any[]>([]); const [categories, setCategories] = useState<any[]>([]);
  ...
  const load = useCallback(async (pg=1)=>{
    setLoading(true);
    try{const d:any=await getBusinesses({status:"rejected",pageSize:50,page:pg,q:q||undefined,area_id:areaId||undefined,category_id:catId||undefined});
  ...
  useEffect(()=>{const t=setTimeout(()=>load(1),300);return()=>clearTimeout(t);},[q,areaId,catId,load]);

  const toggle=(id:string)=>setSelected(p=>{const n=new Set(p);n.has(id)?n.delete(id):n.add(id);return n;});
  const toggleAll=()=>setSelected(p=>p.size===rows.length?new Set():new Set(rows.map(r=>r.id)));

  async function restore(id:string){setBusyId(id);try{await patchBusiness(id,{action:"restore"});load(page);}catch(e:any){setError(e.message);}finally{setBusyId(null);}}
  async function permDelete(id:string){if(!confirm("Permanently delete?"))return;setBusyId(id);try{await deleteBusinessForever(id);setRows(p=>p.filter(r=>r.id!==id));}catch(e:any){setError(e.message);}finally{setBusyId(null);}}
  async function bulkRestore(){setBulkBusy(true);try{await bulkBusinessAction({action:"restore",ids:Array.from(selected)});load(page);}catch(e:any){setError(e.message);}finally{setBulkBusy(false);}}
  async function bulkDeleteForever(){if(!confirm(`Permanently delete ${selected.size}?`))return;setBulkBusy(true);try{await bulkBusinessAction({action:"delete",ids:Array.from(selected)});load(page);}catch(e:any){setError(e.message);}finally{setBulkBusy(false);}}

  return (
    <div>
      <h1 className="mb-4 text-xl font-semibold text-ink-900 dark:text-gray-100">Rejected / Trash</h1>
      <div className="mb-4 flex flex-wrap gap-2">
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search…" className={SEL+" w-56"}/>
        <select value={areaId} onChange={e=>setAreaId(e.target.value)} className={SEL}>
          <option value="">All areas</option>{areas.map((a:any)=><option key={a.id} value={a.id}>{a.name}</option>)}
        </select>
        <select value={catId} onChange={e=>setCatId(e.target.value)} className={SEL}>
          <option value="">All types</option>{categories.map((c:any)=><option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        {(q||areaId||catId)&&<button onClick={()=>{setQ("");setAreaId("");setCatId("");}} className="rounded-lg border border-black/10 px-3 py-2 text-sm text-ink-900/60 hover:bg-gray-50 dark:border-white/10 dark:text-gray-400 dark:hover:bg-gray-800">Clear ✕</button>}
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-left text-xs uppercase text-ink-900/40 dark:bg-gray-950 dark:text-gray-500">
            <tr>
              ...
              <th className="px-4 py-3">Name</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Area</th><th className="px-4 py-3">Reason</th><th className="px-4 py-3 text-right">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-black/5 dark:divide-white/10">
            {loading?<tr><td colSpan={6} className="px-4 py-10 text-center text-ink-900/40 dark:text-gray-500">Loading…</td></tr>
            :rows.length===0?<tr><td colSpan={6} className="px-4 py-10 text-center text-ink-900/40 dark:text-gray-500">Nothing in trash.</td></tr>
            :rows.map(b=>(
              <tr key={b.id} className={`hover:bg-gray-50 dark:hover:bg-gray-800 ${selected.has(b.id)?"bg-brand-50/40 dark:bg-brand-900/10":""}`}>
                ...
                <td className="px-4 py-3 text-ink-900/60 dark:text-gray-400 max-w-xs truncate">{b.rejection_reason||"—"}</td>
                <td className="px-4 py-3">
                  <div className="flex flex-row flex-nowrap items-center justify-end gap-1">
                    <PreviewMapButtons business={b}/>
                    <button disabled={busyId===b.id} onClick={()=>restore(b.id)} className="shrink-0 rounded-lg border border-black/10 px-2.5 py-1 text-xs font-medium text-ink-900 hover:bg-gray-50 disabled:opacity-50 dark:border-white/10 dark:text-gray-200 dark:hover:bg-gray-800">Restore</button>
                    <button disabled={busyId===b.id} onClick={()=>permDelete(b.id)} className="shrink-0 rounded-lg bg-red-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-red-700 disabled:opacity-50">Delete</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <div className="mt-4 flex items-center justify-between">
        <p className="text-xs text-ink-900/40 dark:text-gray-500">{total===0?"No results":`${((page-1)*50)+1}–${Math.min(page*50,total)} of ${total.toLocaleString()}`}</p>
