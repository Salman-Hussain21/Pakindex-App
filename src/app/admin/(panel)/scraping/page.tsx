...
export default function ScrapingCenterPage() {
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-semibold text-ink-900">Scraping Center</h1>
      ...
      <div className="mb-4 flex gap-2">
        <input
          ...
          className="w-full max-w-md rounded-lg border border-black/10 px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500"
        />
        <button
      ...
      {results.length > 0 && (
        <>
          <div className="mb-3 flex items-center justify-between">
            <p className="text-sm text-ink-900/60">{results.length} results found</p>
            <button
          ...
          <div className="overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-black/5">
                {results.map((b, i) => (
                  <tr key={i}>
                    <td className="px-4 py-3 font-medium text-ink-900">{b.title}</td>
                    <td className="px-4 py-3 text-ink-900/60">{b.type || "—"}</td>
                    <td className="px-4 py-3 text-ink-900/60">{b.address || "—"}</td>
