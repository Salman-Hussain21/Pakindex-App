...
export default function PendingApprovalPage() {
  ...
  const load = useCallback(async (pg = 1) => {
    ...
    try {
      const d: any = await getBusinesses({
        status: "pending", pageSize: 50, page: pg,
        q: q || undefined, area_id: areaId || undefined,
        category_id: catId || undefined, min_rating: minRating || undefined,
      });
      setRows(d.businesses || []); setTotal(d.pagination?.total || 0);
      setTotalPages(d.pagination?.totalPages || 1); setPage(pg); setSelected(new Set());
    } catch (e: any) { setError(e.message); } finally { setLoading(false); }
  }, [q, areaId, catId, minRating]);

  useEffect(() => { const t = setTimeout(() => load(1), 300); return () => clearTimeout(t); }, [q, areaId, catId, minRating, load]);
  ...
  async function approve(id: string) {
    setBusyId(id);
    try { await patchBusiness(id, { action: "approve" }); setRows(p => p.filter(r => r.id !== id)); setTotal(t => Math.max(0, t - 1)); }
    catch (e: any) { setError(e.message); } finally { setBusyId(null); }
  }
  async function reject(id: string, reason: string) {
    setBusyId(id);
    try { await patchBusiness(id, { action: "reject", reason }); setRows(p => p.filter(r => r.id !== id)); setTotal(t => Math.max(0, t - 1)); setRejectFor(null); setRejectReason(""); }
    catch (e: any) { setError(e.message); } finally { setBusyId(null); }
  }
  async function bulkApprove() {
    setBulkBusy(true);
    try { await bulkBusinessAction({ action: "approve", ids: Array.from(selected) }); load(page); }
    catch (e: any) { setError(e.message); } finally { setBulkBusy(false); }
  }
  async function bulkReject() {
    setBulkBusy(true);
    try { await bulkBusinessAction({ action: "reject", ids: Array.from(selected), reason: "Bulk rejected" }); load(page); }
    catch (e: any) { setError(e.message); } finally { setBulkBusy(false); }
  }
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        {(q || areaId || catId || minRating) && (
          <button onClick={() => { setQ(""); setAreaId(""); setCatId(""); setMinRating(""); }}
      ...
      <div className="overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm dark:border-white/10 dark:bg-gray-900">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-black/5 dark:divide-white/10">
            {loading ? <tr><td colSpan={7} className="px-4 py-10 text-center text-ink-900/40 dark:text-gray-500">Loading…</td></tr>
            : rows.length === 0 ? <tr><td colSpan={7} className="px-4 py-10 text-center text-ink-900/40 dark:text-gray-500">Nothing pending 🎉</td></tr>
            : rows.map(b => (
              <tr key={b.id} className={`hover:bg-gray-50 dark:hover:bg-gray-800 ${selected.has(b.id) ? "bg-brand-50/40 dark:bg-brand-900/10" : ""}`}>
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-row flex-nowrap items-center justify-end gap-1">
                    ...
                    <button disabled={busyId === b.id} onClick={() => approve(b.id)} className="shrink-0 rounded-lg bg-brand-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-brand-700 disabled:opacity-50">Approve</button>
                    <button disabled={busyId === b.id} onClick={() => setRejectFor(b.id)} className="shrink-0 rounded-lg bg-red-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-red-700 disabled:opacity-50">Reject</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <div className="mt-4 flex items-center justify-between">
        <p className="text-xs text-ink-900/40 dark:text-gray-500">
          {total === 0 ? "No results" : `Showing ${((page-1)*50)+1}–${Math.min(page*50,total)} of ${total.toLocaleString()}`}
        </p>
        {totalPages > 1 && (
          <div className="flex items-center gap-1">
            ...
            <button disabled={page<=1} onClick={() => load(page-1)} className="rounded-lg border border-black/10 px-2.5 py-1 text-xs text-ink-900 hover:bg-gray-50 disabled:opacity-40 dark:border-white/10 dark:text-gray-200 dark:hover:bg-gray-800">‹ Prev</button>
            <span className="px-3 text-xs text-ink-900/60 dark:text-gray-400">Page {page} of {totalPages}</span>
            <button disabled={page>=totalPages} onClick={() => load(page+1)} className="rounded-lg border border-black/10 px-2.5 py-1 text-xs text-ink-900 hover:bg-gray-50 disabled:opacity-40 dark:border-white/10 dark:text-gray-200 dark:hover:bg-gray-800">Next ›</button>
